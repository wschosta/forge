classdef state < forge
    ...
    methods
        function obj=state(varargin)
            ...
            obj.generate_all_categories = true;
        ...
        function run(obj)
            ...
            if exist(sprintf('%s/saved_data.mat',obj.data_directory),'file') ~= 2 || obj.recompute
                ...
                if ~any([list{:}])
                    ...
                    if ~strcmp(obj.state_ID,'IN') && length(util.CStrAinBP({'year','role_id','party_id'},obj.people.Properties.VariableNames)) == 3
                        obj.people.party_id = obj.people.party_id - 1;
                        obj.people          = sortrows(obj.people,'party_id');
                        ...
                        warning('Reading precompiled data for Indiana...')
                        
                        % If we can't find anything, do a hardcode read
                        % (just so there's something here)
                        house_people = readtable(sprintf('%s/undergrad/people_2013-2014.xlsx',obj.data_directory));
                        house_people = sortrows(house_people,'party_id');
                    end
                ...
                if ~isempty(house_people)
                    [house_chamber_matrix,house_chamber_votes,...
                        ...
                        house_republicans_committee_sponsor,house_seat_matrix] = obj.processChamberVotes(house_people,'house',0);
                end
                ...
                if ~isempty(senate_people)
                    [senate_chamber_matrix,senate_chamber_votes,...
                        ...
                        senate_republicans_committee_sponsor,senate_seat_matrix] = obj.processChamberVotes(senate_people,'senate',0);
                end
                
                if obj.generate_all_categories
                    for category = 11:-1:0
                        if ~isempty(house_people)
                            [house_chamber_matrix,house_chamber_votes,...
                                house_sponsor_chamber_matrix,house_sponsor_chamber_votes,...
                                ...
                                house_democrats_chamber_sponsor,house_republicans_committee_votes,...
                                house_democrats_committee_votes,house_democrats_committee_sponsor,...
                                house_republicans_committee_sponsor,house_seat_matrix] = obj.processChamberVotes(house_people,'house',category);
                            ...
                            obj.writeTables(house_chamber_matrix,house_chamber_votes,...
                                ...
                                house_seat_matrix,'house',category);
                            ...
                            plot.plotRunner(obj.show_warnings,obj.outputs_directory,obj.histogram_directory,...
                                ...
                                house_consistency_matrix,category)
                        end
                        ...
                        if ~isempty(senate_people)
                            [senate_chamber_matrix,senate_chamber_votes,...
                                ...
                                senate_democrats_chamber_sponsor,senate_republicans_committee_votes,...
                                senate_democrats_committee_votes,senate_democrats_committee_sponsor,...
                                senate_republicans_committee_sponsor,senate_seat_matrix] = obj.processChamberVotes(senate_people,'senate',category);
                            ...
                            obj.writeTables(senate_chamber_matrix,senate_chamber_votes,...
                                ...
                                senate_seat_matrix,'senate',category);
                            ...
                            plot.plotRunner(obj.show_warnings,obj.outputs_directory,obj.histogram_directory,...
                                ...
                                senate_consistency_matrix,category)
                        end
        ...
        function createCategoryGraphs(obj,chamber)
            if ~isempty(senate_people)
                    plot.plotRunner(obj.show_warnings,obj.outputs_directory,obj.histogram_directory,...
                        'Senate',senate_chamber_matrix,senate_republicans_chamber_votes,...
                        senate_democrats_chamber_votes,senate_sponsor_chamber_matrix,...
