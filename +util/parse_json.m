...
if pos <= len
     ...
     function str = parse_string
        ...
        while pos <= len
            ...
            if index_esc > len_esc
                str = [str string(pos:end)]; %#ok<AGROW>
                pos = len + 1;
                break;
            else
                str = [str string(pos:esc(index_esc)-1)]; %#ok<AGROW>
                pos = esc(index_esc);
            end
            switch string(pos)
                case '"' 
                ...
                case '\'
                    ...
                    switch string(pos)
                        case {'"' '\' '/'}
                            str(end+1) = string(pos); %#ok<AGROW>
                            pos = pos + 1;
                        case {'b' 'f' 'n' 'r' 't'}
                            str(end+1) = sprintf(['\' string(pos)]); %#ok<AGROW>
                            pos = pos + 1;
                        case 'u'
                            if pos+4 > len
    ...
    function num = parse_number
        [num, ~, err, delta] = sscanf(string(pos:min(len,pos+20)), '%f', 1); % TODO : compare with json(pos:end)
        if ~isempty(err)
