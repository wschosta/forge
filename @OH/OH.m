classdef OH < forge
    % OH
    % Subclass Ohio of the Superclass Forge
    % Used to generate the prediction algorithms and data repositiories
    % for the state of Ohio. Driven by Legiscan data.
    %
    % SYNTAX:
    % ---------------------------------------------------------------------
    % OH_obj = OH(varargin)
    ...
    methods
        function obj = OH(varargin)
            ...
            obj.state       = 'OH'; % state
            obj.senate_size = 33;   % number of seats in the Senate (upper chamber)
            obj.house_size  = 99;   % number of seats in the House (lower chamber)
        ...
        function run(obj)
            ...
            if exist(sprintf('%s/saved_data.mat',obj.data_directory),'file') ~= 2 || obj.recompute
                ...
                % CODED SPECIFICALLY FOR THE OHIO HOUSE AND SENATE.
            ...
            % Save out all of the generated data, with the exception of
            % the OH object to the workspace,
            var_list = who;
            var_list = var_list(~ismember(var_list,'obj'));
